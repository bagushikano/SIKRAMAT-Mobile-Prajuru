...
public class PerkawinanCampuranMasukDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_perkawinan_campuran_masuk_detail);
        ...
        perkawinanDetailResponseCall.enqueue(new Callback<PerkawinanDetailResponse>() {
            ...
            public void onResponse(Call<PerkawinanDetailResponse> call, Response<PerkawinanDetailResponse> response) {
                if (response.code() == 200 && response.body().getStatus().equals(true)) {
                    ...
                    if (response.body().getPerkawinan().getAlamatAsalPradana() != null) {
                        alamatAsal.setText(response.body().getPerkawinan().getAlamatAsalPradana().toString());
                    }
                    provinsiAsal.setText(
                            response.body().getPerkawinan().getDesaDinasPradana().getKecamatan().getKabupaten().getProvinsi().getName());
                    kabkotAsal.setText(response.body().getPerkawinan().getDesaDinasPradana().getKecamatan().getKabupaten().getName());
                    kecAsal.setText(response.body().getPerkawinan().getDesaDinasPradana().getKecamatan().getName());
                    desDinasAsal.setText(response.body().getPerkawinan().getDesaDinasPradana().getName());
                    if (response.body().getPerkawinan().getNamaAyahPradana() != null) {
                        namaAyah.setText(response.body().getPerkawinan().getNamaAyahPradana().toString());
                    }
                    if (response.body().getPerkawinan().getNamaIbuPradana() != null) {
                        namaIbu.setText(response.body().getPerkawinan().getNamaIbuPradana().toString());
                    }
                    if (response.body().getPerkawinan().getFileSudhiWadhani() != null) {
                      sudhiWadhani.setOnClickListener(new View.OnClickListener() {
                          ...
                          public void onClick(View view) {
                              ...
                              Long download =   downloadUtil.downloadFile(
                                      getResources().getString(R.string.api_endpoint) + response.body().getPerkawinan().getFileSudhiWadhani());
                              if (download != 0) {
    ...
    public void sahPerkawinan(String token, int id) {
        ...
        saveSahButton.setVisibility(GONE);
        ApiRoute getData = RetrofitClient.buildRetrofit().create(ApiRoute.class);
        Call<PerkawinanDetailResponse> perkawinanDetailResponseCall = getData.perkawinanCampuranMasukSah("Bearer " + token, id);
        perkawinanDetailResponseCall.enqueue(new Callback<PerkawinanDetailResponse>() {
            @Override
            public void onResponse(Call<PerkawinanDetailResponse> call, Response<PerkawinanDetailResponse> response) {
                if (response.code() == 200 && response.body().getStatus().equals(true)) {
