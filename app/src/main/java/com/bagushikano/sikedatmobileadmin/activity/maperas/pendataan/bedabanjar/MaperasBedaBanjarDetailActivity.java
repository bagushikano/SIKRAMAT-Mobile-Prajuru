...
public class MaperasBedaBanjarDetailActivity extends AppCompatActivity {
    ...
    Button showTolakMaperasButton, tolakCancelButton, submitTolakButton, konfirmasiButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_maperas_beda_banjar_detail);
        ...
        tolakAlasanCard = findViewById(R.id.maperas_tolak_card);
        alasanTolakField = findViewById(R.id.maperas_tolak_field);
        showTolakMaperasButton = findViewById(R.id.maperas_tolak_show_button);
        tolakCancelButton = findViewById(R.id.maperas_tolak_cancel_button);
        submitTolakButton = findViewById(R.id.maperas_tolak_submit_button);
        konfirmasiButton = findViewById(R.id.maperas_konfirmasi_button);

        saveSahButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                sahMaperas(loginPreferences.getString("token", "empty"),
                        getIntent().getIntExtra(MAPERAS_DETAIL_KEY, -1));
            }
        });

        showTolakMaperasButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                tolakAlasanCard.setVisibility(View.VISIBLE);
                saveSahButton.setVisibility(GONE);
                showTolakMaperasButton.setVisibility(GONE);
        ...
        tolakCancelButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                saveSahButton.setVisibility(GONE);
                showTolakMaperasButton.setVisibility(View.VISIBLE);
        ...
        konfirmasiButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new MaterialAlertDialogBuilder(MaperasBedaBanjarDetailActivity.this)
                        .setTitle("Konfirmasi Maperas Keluar")
                        .setMessage("Yakin ingin mengkonfirmasi maperas?")
                        .setPositiveButton("Simpan", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                konfirmasiMaperas(loginPreferences.getString("token", "empty"),
                                        getIntent().getIntExtra(MAPERAS_DETAIL_KEY, -1));
                            }
                        })
        ...
        submitTolakButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (alasanTolakField.getText().toString().length() == 0) {
                    ...
                    new MaterialAlertDialogBuilder(MaperasBedaBanjarDetailActivity.this)
                            .setTitle("Maperas Belum Dapat Dikonfirmasi")
                            .setMessage("Yakin ingin tidak mengkonfirmasi maperas?")
                            .setPositiveButton("Simpan", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    tolakMaperas(loginPreferences.getString("token", "empty"),
                                            getIntent().getIntExtra(MAPERAS_DETAIL_KEY, -1));
                                }
                            })
                            .setNegativeButton("Batal", new DialogInterface.OnClickListener() {
            ...
            public void onResponse(Call<MaperasDetailResponse> call, Response<MaperasDetailResponse> response) {
                if (response.code() == 200 && response.body().getStatus().equals(true)) {
                    if (response.body().getMaperas().getStatusMaperas() == 0) {
                        ...
                        if (response.body().getSisiBanjarAdatKeluar()) {
                            saveSahButton.setVisibility(GONE);
                            showTolakMaperasButton.setVisibility(View.VISIBLE);
                            konfirmasiButton.setVisibility(View.VISIBLE);
                        } else {
                    ...
                    } else if(response.body().getMaperas().getStatusMaperas() == 1) {
                        maperasStatusText.setText("Terkonfirmasi");
                        maperasStatusText.setTextColor(getApplicationContext().getColor(R.color.yellow));
                        if (response.body().getSisiBanjarAdatKeluar()) {
                            saveSahButton.setVisibility(GONE);
                        } else {
                        ...
                        showTolakMaperasButton.setVisibility(GONE);
                        konfirmasiButton.setVisibility(GONE);
                    } else if (response.body().getMaperas().getStatusMaperas() == 2) {
                        maperasStatusText.setText("Tidak Terkonfirmasi");
                        if (response.body().getSisiBanjarAdatKeluar()) {
                            saveSahButton.setVisibility(GONE);
                        } else {
                            saveSahButton.setVisibility(GONE);
                            editPerkawinan.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    ...
                                    Intent editPerkawinan = new Intent(getApplicationContext(), MaperasBedaBanjarDataKramaLamaActivity.class);
                                    editPerkawinan.putExtra("EDIT_MAPERAS", gson.toJson(response.body().getMaperas()));
                            ...
                            deletePerkawinan.setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View view) {
                                    deleteMaperas(loginPreferences.getString("token", "empty"), getIntent().getIntExtra(MAPERAS_DETAIL_KEY, -1));
                                }
                            });

                        }
                        penolakanCard.setVisibility(View.VISIBLE);
                        tolakAlasanText.setText(response.body().getMaperas().getAlasanPenolakan().toString());
                        showTolakMaperasButton.setVisibility(GONE);
                        konfirmasiButton.setVisibility(GONE);
    ...
    public void deleteMaperas(String token, int id) {
        ...
        konfirmasiButton.setVisibility(GONE);
        ApiRoute getData = RetrofitClient.buildRetrofit().create(ApiRoute.class);
        Call<MaperasDetailResponse> perkawinanDetailResponseCall = getData.maperasBedaBanjarDelete("Bearer " + token, id);
        perkawinanDetailResponseCall.enqueue(new Callback<MaperasDetailResponse>() {
            @Override
            public void onResponse(Call<MaperasDetailResponse> call, Response<MaperasDetailResponse> response) {

                maperasProgressLayout.setVisibility(View.GONE);
                Toast.makeText(getApplicationContext(), "Data Maperas berhasil dihapus!", Toast.LENGTH_LONG).show();
                finish();
            }
    ...
    public void konfirmasiMaperas(String token, int id) {
        maperasProgressLayout.setVisibility(View.VISIBLE);
        tolakAlasanCard.setVisibility(GONE);
        saveSahButton.setVisibility(GONE);
        showTolakMaperasButton.setVisibility(GONE);
        konfirmasiButton.setVisibility(GONE);
        ApiRoute getData = RetrofitClient.buildRetrofit().create(ApiRoute.class);
        Call<MaperasDetailResponse> maperasDetailResponseCall = getData.maperasBedaBanjarKonfirmasi("Bearer " + token, id);
        maperasDetailResponseCall.enqueue(new Callback<MaperasDetailResponse>() {
            @Override
            public void onResponse(Call<MaperasDetailResponse> call, Response<MaperasDetailResponse> response) {
                if (response.code() == 200 && response.body().getStatus().equals(true)) {
                    Snackbar.make(getWindow().getDecorView().findViewById(android.R.id.content), "Maperas berhasil dikonfirmasi.", Snackbar.LENGTH_SHORT).show();
                }
                else {
                    Snackbar.make(getWindow().getDecorView().findViewById(android.R.id.content), R.string.server_fail, Snackbar.LENGTH_SHORT).show();
                }
    ...
    public void tolakMaperas(String token, int id) {
        maperasProgressLayout.setVisibility(View.VISIBLE);
        tolakAlasanCard.setVisibility(GONE);
        saveSahButton.setVisibility(GONE);
        showTolakMaperasButton.setVisibility(GONE);
        konfirmasiButton.setVisibility(GONE);
        ApiRoute getData = RetrofitClient.buildRetrofit().create(ApiRoute.class);
        Call<MaperasDetailResponse> maperasDetailResponseCall = getData.maperasBedaBanjarTolak(
                "Bearer " + token,
                id,
        ...
        maperasDetailResponseCall.enqueue(new Callback<MaperasDetailResponse>() {
            ...
            public void onResponse(Call<MaperasDetailResponse> call, Response<MaperasDetailResponse> response) {
                if (response.code() == 200 && response.body().getStatus().equals(true)) {
                    Snackbar.make(getWindow().getDecorView().findViewById(android.R.id.content), "Maperas telak tidak dikonfirmasi.", Snackbar.LENGTH_SHORT).show();
                }
                else {
                    Snackbar.make(getWindow().getDecorView().findViewById(android.R.id.content), R.string.server_fail, Snackbar.LENGTH_SHORT).show();
                }
    ...
    public void sahMaperas(String token, int id) {
        ...
        konfirmasiButton.setVisibility(GONE);
        ApiRoute getData = RetrofitClient.buildRetrofit().create(ApiRoute.class);
        Call<MaperasDetailResponse> maperasDetailResponseCall = getData.maperasBedaBanjarSah("Bearer " + token, id);
        maperasDetailResponseCall.enqueue(new Callback<MaperasDetailResponse>() {
            @Override
            public void onResponse(Call<MaperasDetailResponse> call, Response<MaperasDetailResponse> response) {
                if (response.code() == 200 && response.body().getStatus().equals(true)) {
